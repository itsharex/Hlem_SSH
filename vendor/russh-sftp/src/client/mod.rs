...
pub struct Config {
    ...
    /// Upper bound for the number of concurrent in-flight read requests.
    /// Default: 128.
    ///
    /// Mirror of [`Self::max_concurrent_writes`] in spirit, but the read path
    /// uses *adaptive* pipelining: the actual depth starts small (8) and grows
    /// by one per completed READ response, capped at this value. This lets
    /// short-lived or low-bandwidth connections avoid blasting the SSH window
    /// with 128 requests up front while still saturating high-BDP links.
    /// Setting this to 1 forces strict serial reads (legacy behaviour).
    ///
    /// [`File`]: crate::client::fs::File
