...
impl RawSftpSession {
    ...
    ///
    /// Mirror of [`Self::write_nowait`]. Returns a [`oneshot::Receiver`] that
    /// resolves to either a `Data` packet (with up to `len` bytes), a `Status`
    /// packet (e.g. `Eof`), or an error. Used by [`File`]'s pipelined
    /// `AsyncRead` implementation to keep multiple READ requests in flight.
    ///
