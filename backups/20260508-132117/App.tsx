...
import { App as AntdApp, Button, ConfigProvider, Modal, theme } from "antd";
...
const SplitPane = lazy(() => import("./components/SplitPane").then((module) => ({ default: module.SplitPane })));
const SessionConfigModal = lazy(() => import("./components/SessionConfigModal").then((module) => ({ default: module.SessionConfigModal })));
const TelemetrySidebar = lazy(() => import("./components/TelemetrySidebar").then((module) => ({ default: module.TelemetrySidebar })));
const TerminalPanel = lazy(() => import("./components/TerminalPanel").then((module) => ({ default: module.TerminalPanel })));
const TopBar = lazy(() => import("./components/TopBar").then((module) => ({ default: module.TopBar })));
const TransferCenter = lazy(() => import("./components/TransferCenter").then((module) => ({ default: module.TransferCenter })));
const TunnelDrawer = lazy(() => import("./components/TunnelDrawer").then((module) => ({ default: module.TunnelDrawer })));

type VaultMode = "loading" | "create" | "unlock" | "ready";
type SessionModalState = { mode: "create"; input: SessionInput } | { mode: "edit"; sessionId: string; input: SessionInput };
const CWD_TRACKING_COMMAND =
  "export PROMPT_COMMAND='printf \"\\033]777;cwd=%s\\007\" \"$PWD\"'; printf '\\033]777;cwd=%s\\007' \"$PWD\"\n";
...
function App() {
  ...
  function applySnapshot(snapshot: ConfigSnapshot, preferredSessionId?: string) {
    ...
    const preferredId =
      preferredSessionId && mappedIds.includes(preferredSessionId)
        ? preferredSessionId
        : mappedIds[0] ?? "";
  ...
  async function closeSession(id: string) {
    ...
    if (session?.connectionId) {
      try {
        ...
        appendTerminal(session.id, "error", getErrorMessage(error));
      }
    }
    setOpenSessionIds((current) => {
  ...
  function toggleSessionConnection(id: string) {
    activateSession(id);
    const session = sessions.find((item) => item.id === id);
    if (!session || connectingSessionId === id) return;
  ...
  function upsertTransfer(payload: TransferInfo) {
    setTransfers((current) => {
      ...
      if (existing === -1) return [payload, ...current].slice(0, 8);
      const next = [...current];
  ...
  async function retryTransfer(transferId: string) {
    try {
      const next = await remoteApi.retryTransfer(transferId);
      setTransfers((current) => [next, ...current.filter((transfer) => transfer.transferId !== transferId)]);
    } catch (error) {
      appendTerminal(activeSessionId, "error", getErrorMessage(error));
    }
  }
  ...
  function handleTerminalOutput(payload: TerminalOutputEvent) {
    ...
    const entry = createTerminalEntry(payload.kind, data);
    if (!entry.content) return;
  ...
  function installCwdTracking(terminalId: string) {
    ...
    void remoteApi.writeTerminal(terminalId, CWD_TRACKING_COMMAND).catch(() => undefined);
  }
  ...
  async function connectSession(session = activeSession) {
    ...
    try {
      ...
      const [sftpResult, terminalResult] = await Promise.all([
        openSftpWithFiles(connectionId, initialPath, session.username),
        remoteApi
      ...
      const files = sftpResult.files;
      const terminal = terminalResult.value;

      if (sftpResult.error) warnings.push(`SFTP 不可用：${sftpUnavailableMessage(sftpResult.error)}`);
      if (terminalResult.error) warnings.push(`终端不可用：${getErrorMessage(terminalResult.error)}`);
      ...
      const [telemetryJob, initialTelemetry] = await Promise.all([
  ...
  async function sendTerminalData(data: string) {
    ...
    try {
      ...
      appendTerminal(activeSession.id, "error", getErrorMessage(error));
    }
  }
  ...
  async function refreshFiles(sftpId: string, path: string, sessionId: string) {
    try {
      ...
      appendTerminal(sessionId, "error", getErrorMessage(error));
    }
  }
  ...
  function getHostKeyPayload(error: unknown): HostKeyVerification | null {
            ...
            <Suspense fallback={<AppLoadingFallback />}>
              <>
                <TopBar
                  ...
                  onTabToggle={toggleSessionConnection}
                      ...
                      <SplitPane
                        ...
                        bottom={
                          <FileManager
                            ...
                            transfers={transfers.filter((transfer) => transfer.sftpId === activeSession.sftpId)}
                            onTransferOpen={() => setTransferCenterOpen(true)}
                          />
                        }
                      />
                  ...
                  <section className="emptyWorkspace">
                    <div className="emptyWorkbench">
                      {sessions.length > 0 ? (
                        <>
                          <h2>请选择会话</h2>
                          <p>已保存 {sessions.length} 个 SSH 连接，点击下方按钮选择并连接。</p>
                          <div className="emptyWorkbenchActions">
                            <Button type="primary" icon={<ApiOutlined />} onClick={() => setSessionListOpen(true)}>
                              打开会话列表
                            </Button>
                            <Button icon={<PlusOutlined />} onClick={() => void addSession()}>
                              新建会话
                            </Button>
                          </div>
                        </>
                        ...
                        <>
                          <h2>暂无会话</h2>
                          <p>新建一个 SSH 会话开始使用。</p>
                          <Button type="primary" icon={<PlusOutlined />} onClick={() => void addSession()}>
                            新建会话
                          </Button>
                        </>
                      )}
...
function sessionConfigToInput(config: SessionConfig): SessionInput {
  return {
    name: config.name,
...
function createNextSessionName(sessions: SessionConfig[], excludeId?: string) {
...
function shouldSkipTerminalEntry(entries: TerminalEntry[], entry: TerminalEntry) {
...
async function runUploadQueue<T>(
  ...
  const results: T[] = [];
  let cursor = 0;
  const workerCount = Math.max(1, Math.min(8, concurrency, items.length));
  await Promise.all(
...
function uploadConcurrency(count: number) {
  const cores = typeof navigator === "undefined" ? 4 : navigator.hardwareConcurrency || 4;
  if (count <= 1) return 1;
  if (count <= 3) return Math.min(count, 2);
  if (count <= 8) return Math.min(count, Math.max(2, Math.floor(cores / 2)));
  return Math.min(8, Math.max(4, Math.floor(cores * 0.75)));
}

function shouldRunAutoBackup(settings: BackupSettings, records: { status: string; createdAt: string }[]) {
...
function appendTerminalStreamEntries(entries: TerminalEntry[], nextEntries: TerminalEntry[]) {
  return nextEntries.reduce((current, entry) => appendTerminalStreamEntry(current, entry), entries);
}

function stripCwdMarkers(data: string) {
...
function remoteSessionPath(session: Pick<RemoteSession, "username" | "currentPath">) {
  return initialRemotePath(session.username, session.currentPath);
}

async function resolveSftpOperationTarget(sftpId: string, currentPath: string, sourcePath: string, value: string) {
...
function hasTelemetryData(telemetry: ServerTelemetry) {
  return Boolean(
    telemetry.uptime !== "未知" ||
      telemetry.ip ||
      telemetry.cpu > 0 ||
...
function sftpUnavailableMessage(error: unknown) {
